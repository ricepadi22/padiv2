...
import { Bot, Check, Copy, Trash2, Zap, Settings } from "lucide-react";
import { padisApi, type PadiHostBot } from "../../api/padis.ts";
import { botsApi } from "../../api/bots.ts";
import { ProviderConfigForm } from "../bots/ProviderConfigForm.tsx";
      ...
      <div className="space-y-4">
        <div className="border border-dashed border-zinc-300 rounded-xl p-6 text-center">
          ...
          <p className="text-xs text-zinc-400 mt-1 max-w-xs mx-auto">
            An AI host is a community bot powered by Claude that joins all your padi's rooms and helps members.
          </p>
          {isOwner && (
            <button
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 p-3 bg-amber-50 rounded-xl border border-amber-200">
          ...
          <p className="text-xs text-amber-800">
            The AI host will join all existing rooms in <strong>{padiName}</strong> and auto-join future rooms.
          </p>
        </div>
        <input
          autoFocus
    ...
    return (
      <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="text-xs text-zinc-400">{hostBot.status} · {hostBot.provider}</span>
                </div>
              </div>
            </div>
            {isOwner && (
              <div className="flex items-center gap-1.5">
                ...
                <button
                  onClick={() => {
                    if (confirm(`Remove ${hostBot.displayName} as AI host? This will remove them from all padi rooms.`)) {
                      removeHost.mutate();
                    }
                  }}
        ...
        <p className="text-xs text-zinc-400">
          The AI host automatically joins all rooms in this padi and responds to messages using its configured model.
        </p>
      </div>
    );
  }
